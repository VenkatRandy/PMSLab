...
export class EditmodeComponent implements OnInit {
  form: FormGroup;
  templateData = JSON.parse(decodeURI(localStorage.getItem('Template Data')));
  showIndex: number;
  ...
  patch() {
    ...
    const catData = this.templateData.categoryResponses;
  ...
  captureTable(idx, index, data) {
    this.injectHeader(idx, index, data);
    this.injectValueList(idx, index, data);
  }
  ...
  injectValueList(idx, index, data) {
    // Inject Values
  ...
  injectDropdownFromHeader(idx, index, indexx, indexy) {
    ...
    tableH.removeControl('dropDownDefaultResponses');
  }
  ...
  addTableValues(i, z) {
    let table = <FormArray>this.form.get(`categoryRequests.${z}.subCategoryRequests.${i}.tableRequest.tableHeaderRequests`)
    if (table.value != null) {
  ...
  onSubmit() {
    ...
    //   this.spinner.hide();
    //   this.status = data.data.responseStatus;
    //   setTimeout(() => {
    //     // window.location.reload();
    // this.route.navigate(['initiation/process'], { queryParams: { 'reviewId': this.reviewId } });
    //   }, 2000);
    // }, error => {
    //   this.status = error.responseStatus;
    //   this.spinner.hide();
    //   this.submitted = false;
    //   setTimeout(() => {
    //     window.location.reload();
